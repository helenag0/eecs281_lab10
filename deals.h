...
cost best_price(const std::vector<cost>& prices) {
    ...
    std::vector<cost> sortedPrices = prices;
    std::sort(sortedPrices.begin(), sortedPrices.end());

    // total cost, number of punches, and number of coupons used
    int total = 0;
    int punches = 0;

       // go through each meal price
    for (uint32_t i = 0; i < prices.size(); i++) {
           // if we have a punch card, add a hole punch and use the loyalty program discount
           if (punches < 5 && prices.size() > 5)
           {
               total += prices[i];
               punches++;
           }
           else if(punches < 5 && prices.size() < 5) {
               total += discounted(sortedPrices[i]);
           }
           else if(punches == 5) {
               punches = 0;
               continue;
           }
           // otherwise, use a coupon and apply the 25% discount
           else {
               total += discounted(sortedPrices[i]);
           }
